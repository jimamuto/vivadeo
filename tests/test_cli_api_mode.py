...
from vivadeo.cli import cli


def test_stats_uses_api_mode(monkeypatch):
    monkeypatch.setenv("VIVADEO_API_URL", "http://api.test")
    ...
    monkeypatch.setattr("vivadeo.cli._api_request", fake_request)
...
def test_index_uploads_file_in_api_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("vivadeo.cli._api_upload", fake_upload)
