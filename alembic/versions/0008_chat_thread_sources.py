...
def upgrade() -> None:
    ...
    if "chat_thread_videos" not in inspector.get_table_names():
        op.create_table(
            "chat_thread_videos",
    ...
    if "ix_chat_thread_videos_organization" not in {index["name"] for index in inspect(op.get_bind()).get_indexes("chat_thread_videos")}:
        op.create_index(
