...
import type { Job, Video, VideoChunk } from "~/components/dashboard-data";
...
export function IngestPanel({ workspace = "default-workspace" }: { workspace?: string }) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      navigate({ to: `/jobs?job=${encodeURIComponent(job.id)}` as any });
            ...
            {interruptedJobs.map((job) => (
              <article key={job.id} className="detail-card">
                ...
                <div className="dashboard-panel-links">
                  ...
                  <Link to={`/jobs?job=${encodeURIComponent(job.id)}`} className="button-secondary">Open job</Link>
                </div>
              </article>
            ))}
          </div>
      ...
      <article className="card dashboard-panel library-list-panel">
        ...
        {filteredVideos.length === 0 ? (
          <div className="empty-state">
            ...
            <Link to={"/dashboard/ingest" as any} className="button">Open ingest</Link>
          </div>
        ) : (
      ...
      <article className="card dashboard-panel library-detail-panel">
        ...
        {!selectedVideo ? <p className="muted">Select video to inspect details.</p> : (
          <div className="dashboard-stack library-detail-body">
            ...
            <div className="dashboard-panel-links">
              <Link to={`/jobs?job=${encodeURIComponent(latestJobByVideo.get(selectedVideo.id)?.id ?? "")}`} className="button-secondary">Open latest job</Link>
              <Link to="/search" className="button-secondary">Ask about this video</Link>
            </div>
          </div>
        )}
      </article>
  ...
  return (
    <section className="card dashboard-panel workspace-management-panel">
      ...
      <div className="dashboard-panel-links workspace-actions">
        <Link to={"/dashboard/library" as any} className="button-secondary">Open library</Link>
        <Link to="/settings" className="button-secondary">Open settings</Link>
      </div>
    </section>
  );
}
