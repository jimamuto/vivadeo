...
- Modal inference in `vivadeo/modal_app.py` supplies Qwen visual embeddings. Transcription uses Azure OpenAI Whisper through `vivadeo/azure_whisper.py`; Vivadeo Auto chat uses the Azure OpenAI GPT-5.6 Luna deployment configured through the server-side OpenAI-compatible settings, and NVIDIA transcript embeddings are preferred when configured. After any change to `vivadeo/modal_app.py`, automatically run Modal deploy as the verification/deploy step. On Windows, prefer `PYTHONIOENCODING=utf-8 modal deploy vivadeo/modal_app.py` because plain deploy can fail with `'charmap' codec can't encode character '\u2713'` when Modal prints checkmark/emoji characters.
...
- After any frontend change, restart the web frontend container with `docker compose -f docker-compose.dev.yml restart web` before reviewing or visually verifying the change.
